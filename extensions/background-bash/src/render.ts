...
export const renderProcessResult = (
  ...
  const content = resultText(display.result);
  ...
  const details = [log, controls].filter(Boolean).join("\n");
...
export const renderCompletionMessage = (
  ...
  const display = stripModelOnlyLogLine(content, details.logPath);
  const separator = display.indexOf("\n\n");
  const output = separator === -1 ? "" : display.slice(separator + 2).trim();
  const label = details.name ?? "Background bash";
  ...
  const body = [`  $ ${details.command}`, output, logPath].filter(Boolean).join("\n\n");
...
export const renderProcessCall = (
  ...
  const suffix = [args.action, args.pgid].filter((value) => value !== undefined).join(" ");
  return new Text(
    `${theme.fg("toolTitle", theme.bold(toolName))}${suffix ? ` ${theme.fg("dim", suffix)}` : ""}`,
    0,
    0,
  );
};
