...
const snapshotOriginalState = (state: PresetState, ctx: ExtensionContext, pi: ExtensionAPI): void => {
...
const applyPresetTools = (name: string, preset: Preset, ctx: ExtensionContext, pi: ExtensionAPI): void => {
...
const updateStatus = (state: PresetState, ctx: ExtensionContext): void => {
	const value = state.activeName ? ctx.ui.theme.fg("accent", `preset:${state.activeName}`) : undefined;
...
const selectPresetName = async (
	...
	const choices = ["(none)", ...names.map((name) => (name === state.activeName ? `${name} (active)` : name))];
...
const restorePresetName = (options: ResolvedOptions, state: PresetState, ctx: ExtensionContext): void => {
