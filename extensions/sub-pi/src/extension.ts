...
const indentText = (text: string, indent: number): string => {
  return text
    .split("\n")
    .map((line) => indentLine(line, indent))
    .join("\n");
};
...
const getToolCallLines = (messages: Message[], theme: Theme): string[] => {
  ...
  return items.map(
    (item) => `${themeFg("muted", "→ ")}${formatToolCall(item.name, item.args, themeFg)}`,
  );
};
...
const buildTaskBlockLines = (options: {
  label: string;
  result: SingleResult;
  theme: Theme;
  indent: number;
  ...
  const lines = [
    indentLine(`${theme.fg("toolTitle", label)} ${getStatusIcon(status, theme)}`, indent),
  ];
...
const formatAvailableSkills = (
  ...
  return {
    text: listed.map((skill) => `${skill.name} (${skill.sourceInfo.source})`).join(", "),
    remaining,
  };
};
...
const runSingleTask = async (options: {
  ...
  try {
    const args = [
      ...applyForkSessionArgs(options.subprocessArgs, forkSession),
      options.subprocessPrompt,
    ];
    ...
    const exitCode = await new Promise<number>((resolve) => {
      ...
      const processLine = (line: string) => {
        ...
        if (
          (typeText === "message_end" || typeText === "tool_result_end") &&
          isMessage(messageValue)
        ) {
...
const ModelOverrideSchema = Type.Optional(
  Type.String({ description: "Optional model override: provider/modelId" }),
);
...
const TaskItemSchema = Type.Object({
  ...
  fork: Type.Optional(
    Type.Boolean({ description: "Fork context from current session (default: true)" }),
  ),
});
...
const renderParallelResult = (results: SingleResult[], _expanded: boolean, theme: Theme): Text => {
  ...
  const lines = [
    ...
    formatStatusLine(
      status,
      2,
      status === "Running" ? `${doneCount}/${results.length} done` : undefined,
    ),
    indentLine("Tasks:", 2),
  ];
  ...
  for (let index = 0; index < results.length; index++) {
    ...
    lines.push(
      ...buildTaskBlockLines({ label: "sub-pi", result: results[index], theme, indent: 4 }),
    );
  }
...
const renderChainResult = (results: SingleResult[], _expanded: boolean, theme: Theme): Text => {
  ...
  for (let index = 0; index < results.length; index++) {
    ...
    lines.push(
      ...buildTaskBlockLines({ label: `Step ${stepNumber} (sub-pi)`, result, theme, indent: 4 }),
    );
  }
