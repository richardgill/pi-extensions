...
const normalizeForkInput = (
  ...
  if (typeof value !== "boolean")
    return { ok: false, error: `Invalid parameters: ${label} must be a boolean.` };
  return { ok: true, value };
};

const parseTaskItems = (
  rawTasks: unknown[],
...
export const normalizeTaskParams = (
  ...
  if (typeof mode !== "string")
    return { ok: false, error: 'Invalid parameters: "type" must be a string.' };
  ...
  if (mode === "single") {
    if (rawTasks.length !== 1) {
      return {
        ok: false,
        error: 'Invalid parameters: type="single" requires exactly one task in "tasks".',
      };
    }
  ...
  if (mode === "parallel") {
    if (rawTasks.length === 0) {
      return {
        ok: false,
        error: 'Invalid parameters: type="parallel" requires a non-empty "tasks" array.',
      };
    }
  ...
  if (mode === "chain") {
    if (rawTasks.length === 0) {
      return {
        ok: false,
        error: 'Invalid parameters: type="chain" requires a non-empty "tasks" array.',
      };
    }
    if (rawTasks.length > options.maxParallelTasks) {
      return {
  ...
  return {
    ok: false,
    error: 'Invalid parameters: "type" must be "single", "chain", or "parallel".',
  };
};
